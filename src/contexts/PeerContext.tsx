...
export type PeerContextValue = {
  ...
  sendPeersMessage: (msg:unknown, filter:(value:DataConnection, index:number, connections:DataConnection[])=>DataConnection[])=>void
  onDataReceived: (callback:(connection:DataConnection) =>void, id:string) => void
  onPeerConnect: (callback:(connection:DataConnection)=>void, id:string) => void
  peerReady: boolean
...
const PeerContextProvider = ({ children }:{children:React.ReactNode}): React.ReactNode => {
  ...
  const [onConnectSendValue, setOnConnectSendValue] = useState<unknown | null>(null)
  const peerRef = useRef<{value:Peer|null}>({value:null})

  const callbackRef = useRef<Record<string, (...args: DataConnection[]) => unknown>>({})
  ...
  const onPeerConnectRef = useRef<{values:Record<string, (...args: DataConnection[]) => void>}>({})
  ...
  const initialize = useCallback((func?:()=>void) => {
    ...
    const initializePeer = (id: string): Peer => {
      ...
      newPeer.on('connection', (conn) => {
        ...
        conn.on('close', () => {
          setConnections((prev) =>
            prev.filter((connection) => connection.connectionId !== conn.connectionId)
          )
        })
      })

      newPeer.on('error', (err: { type: string; message: string }): Peer | void => {
        console.warn(err)
        switch (err.type) {
          case 'unavailable-id': {
  ...
  }, [myPeerId, addNotification, callbackRef, onConnectSendValue, onPeerConnectRef, peerInitialized, ])
    ...
    (peerId:string, req:unknown) => {
      ...
      function connectToPeer(){
        ...
        conn.on('open', () => {
          ...
          conn.on('data', (data) => {
            const { type, payload } = data as { type: string; payload: { deviceName: string } }
            Object.values(callbackRef.current).forEach((cb) => cb({ type, payload }))
            if (type === 'connection_accepted') {
              addNotification({
      ...
      // connectToPeer();
      if(!peerInitialized) { initialize(connectToPeer);}else{connectToPeer();}
      
    },
    [addNotification, callbackRef, peerInitialized, initialize, ]
  )

  const sendPeersMessage = useCallback(
    (msg:unknown, filter:(value:DataConnection, index:number, connections:DataConnection[])=>DataConnection[]) => {
