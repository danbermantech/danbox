...
import usePeerDataReceived, {
  PeerDataCallbackPayload,
} from '$hooks/useDataReceived';
...
const Trivia = () => {
  ...
  const triviaTimeLimit = useAppSelector(
    (state) => state.game.triviaTimeLimit ?? 30,
  );
  const triviaQuestion = useMemo(() => {
    const availableQuestions = TriviaQuestions.filter((q) =>
      !nsfw ? q.nsfw !== true : true,
    );
      ...
      players.forEach((player) => {
        if (playerAnswers[player.id]) {
          ...
          disp(
            setPlayerInstructions({
              playerId: player.id,
              instructions: triviaQuestion.question,
            }),
          );
          ...
          disp(setPlayerControls({ playerId: player.id, controls: answers }));
        }
      });
    });
  }, [
  ...
  const dataReceivedCallback = useCallback(
  ...
  useEffect(() => {
    ...
    if (timeRemaining <= 0) {
      setResults((prev) => {
        ...
        players.forEach((player) => {
          retVal[player.id] = playerAnswers[player.id] == triviaQuestion.answer;
        });
        return retVal;
      });
      return;
  ...
  const endTrivia = useCallback(() => {
    ...
    const t = setTimeout(() => {
      ...
      if (completed) return;
      players
        .filter((player) => results[player.id])
        .forEach((winner) => {
  ...
  return (
    <div className="flex flex-col w-full items-center max-w-[50dvw] transition-all">
      <div className="fixed top-8 right-10">
        {!completed && (
          <span
            className={`font-titan text-4xl font-bold ${timeRemaining <= 5 ? 'text-red-600 animate-pulse' : 'text-black'}`}
          >
            {timeRemaining}s
          </span>
        )}
      </div>
      <h1 className="font-titan text-6xl text-black pb-4">Trivia Time!</h1>
      <div className="bg-slate-100 border-black rounded-t-xl w-full flex flex-col gap-2 p-2 bg-opacity-85 border-4">
        <div className="flex flex-row justify-between items-center">
          <h1 className="text-4xl font-titan font-bold text-black pb-2">
      ...
      <div className="grid grid-flow-dense gap-2 bg-slate-100 border-black rounded-b-xl w-full bg-opacity-85 p-8 border-4">
        {completed && (
          <div className="flex flex-col gap-4">
            <h1 className="text-4xl text-black text-center">
              Correct Answer: {triviaQuestion.answer}
            </h1>
            <div className="flex flex-row gap-2">
              {players.map((player) => {
                return (
                  <div
                    key={player.name}
                    className="w-full flex items-center justify-items-center content-center justify-center"
                  >
                    <PlayerCard
                      player={{ ...player }}
                      showPoints={true}
                      showGold={true}
                      className={
                        playerAnswers[player.id] == triviaQuestion.answer
        ...
        {!completed &&
          answers.map(({ label }, index) => (
            <div
              key={label as string}
              className="text-center font-titan w-full mx-auto border-2 animate-fill-forwards border-white rounded-xl animate-shake animate-infinite animate-duration-2000 animate-ease-linear px-4 overflow-hidden text-ellipsis flex justify-center items-center select-none min-w-96 bg-black text-white"
              style={{
