...
function seedAssets(
  count: number,
): { x: number; y: number; id: string; collected: boolean | string }[] {
  return Array(count)
    .fill(0)
    .map(() => ({
...
export const Frenzy = (props?: { gold?: number; points?: number }) => {
  ...
  const [gold, setGold] = useState(
    seedAssets(
      props?.gold ??
        Math.floor(Math.random() * (MAX_GOLD - MIN_GOLD + 1) + MIN_GOLD),
    ),
  );
  ...
  const _givePlayerGold = useCallback(
    (playerId: string, gold: number) => {
      setPlayerGold((prev) => {
        return {
          ...prev,
  ...
  const _givePlayerPoints = useCallback(
  ...
  const handlePointCollected = useCallback(
    (playerId: string, assetId: string) => {
      setPoints((prev) => {
        const next = [...prev];
        next[next.findIndex((point) => point.id == assetId)].collected =
          playerId;
        return next;
      });
  ...
  const handleGoldCollected = useCallback(
    (playerId: string, assetId: string) => {
      setGold((prev) => {
        const next = [...prev];
        next[next.findIndex((point) => point.id == assetId)].collected =
  ...
  useEffect(() => {
    if (
      gold.filter((g) => !g.collected).length +
        points.filter((g) => !g.collected).length ==
        0 &&
      results.length == 0
    ) {
      // console.log(results)
  ...
  }, [gold, points, players, playerGold, playerPoints, results]);
  ...
  if (results.length) {
    ...
    return (
      <div className="flex gap-8 flex-col items-center justify-center h-full">
        <h1 className="text-black text-center text-8xl font-extrabold">
          RESULTS
        </h1>
        <div className="flex flex-col rounded-xl overflow-clip divide-y divide-black border-black border w-full max-w-lg">
          {ranked.map((result, i) => (
            <LeaderboardItem
              ...
              player={{
                ...result,
                points: playerPoints[result.id] ?? 0,
                gold: playerGold[result.id] ?? 0,
              }}
              rank={i + 1}
              className={
                i === 0
